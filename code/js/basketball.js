...
// Initialize the two svg legends
var legend = d3.select("#legend_1")
    .append("svg")
    .attr("width", width)
    .attr("height", 30);
...
d3.selectAll("input").on("change", function change() {
    // First check to see which graph was selected to initialize the
    // variables
    if (this.id.split('_')[1] === '1'){
	var transition_svg = svg;
	var player = document.querySelector("#player_holder_1 input:checked").value;
	var graph = document.querySelector("#graph_holder_1 input:checked").value;
	var type = document.querySelector("#type_holder_1 input:checked").value;
	var legend_type = legend;
    }
    else{
	var transition_svg = svg_2;
	var player = document.querySelector("#player_holder_2 input:checked").value;
	var graph = document.querySelector("#graph_holder_2 input:checked").value;
	var type = document.querySelector("#type_holder_2 input:checked").value;
	var legend_type = legend_2;
    }

    // Which data_set do we use?
    if (player === "kobe"){
	var data = kobe;
    }
    else{
    ...
    change_legend(player,legend_type);
    // Now we need to know what function to call
    if (graph === "stacked_area"){
	transition_stack_area(data,transition_svg,player,type);
    }
    else if (graph === "stacked_rect"){
	transition_stack_rects(data,transition_svg,player,type);
    }
    else{
	transition_group_rects(data,transition_svg,player,type);
    }
});

// Lets read in the kobe bryant json and display
change_legend("kobe",legend);
transition_stack_area(kobe,svg, "kobe", "percentage");




change_legend("taylor",legend_2);
transition_stack_rects(taylor,svg_2,"taylor","points");


// Use this helper function to change the legend depending on what is
// selected
function change_legend (player, svg){
    ...
    if (player === "kobe"){
	var player_text = "Kobe Bryant";
    }
    else{
	var player_text = "Jack Taylor";
    }
    ...
    var svg_text = svg.selectAll(".player_text")
	.data([player_text,"Rest of team"]);
	...
	.attr("color","black")
	.attr("class","player_text")
	.attr("x",function(d,i){
	    return x_scale(i) + 1.5*length;})
	.attr("y",length);
}
